...
Operator::Operator(std::string name, const std::vector<Variable*>& inputs_, size_t num_outputs, Graph* graph_) : inputs(inputs_), graph(graph_) {
...
std::vector<Variable*> call(const std::string& name, const std::vector<Variable*>& vs) {
  HMA_ENFORCE(vs.size());
